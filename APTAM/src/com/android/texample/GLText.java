...
public class GLText {
   ...
   public GLText( AssetManager assets) {
      ...
      batch = new SpriteBatch(textshader, CHAR_BATCH_SIZE );  // Create Sprite Batch (with Defined Size)
   ...
   public void begin(int shaderid)  {
	   //GLES20.glUseProgram(textshader);
   ...
   public void end()  {
      batch.endBatch(textshader);                               // End Batch
   ...
   public void draw(String text, float x, float y)  {
      ...
      y += ( chrHeight / 2.0f ) - ( fontPadY * scaleY ) - chrHeight/2.0f;  // Adjust Start Y
      for ( int i = 0; i < len; i++ )  {              // FOR Each Character in String
         int c = (int)text.charAt( i ) - CHAR_START;  // Calculate Character Index (Offset by First Char in Font)
         if ( c < 0 || c >= CHAR_CNT )                // IF Character Not In Font
            c = CHAR_UNKNOWN;                         // Set to Unknown Character Index
         batch.drawSprite( x, y, chrWidth, chrHeight, charRgn[c] ,textshader);  // Draw the Character
