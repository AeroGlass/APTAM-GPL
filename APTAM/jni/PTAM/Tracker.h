...
{
public:
  Tracker(CVD::ImageRef irVideoSize, const ATANCamera &c, Map &map, MapMaker &mm);
  ~Tracker();
  
  // TrackFrame is the main working part of the tracker: call this every frame.
  void TrackFrame(CVD::Image<CVD::byte> &imFrame,CVD::Image<CVD::Rgb<CVD::byte>> &imFrameColor, bool bDraw);
...
protected:
  ...
  SO3<> firstrot;
  SO3<> firstglobal;
  void getRotation(double * rotMat);
