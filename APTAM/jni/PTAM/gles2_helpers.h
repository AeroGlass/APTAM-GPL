/*
    ...
    modified: ICGJKU 2015
*/
#ifndef CVD_GLES2_HELPERS_H
#define CVD_GLES2_HELPERS_H
...
namespace CVD
{
	...
	{
		gles2h.glTranslatef(v[0], v[1], v[2]);
	}
	...
	{
		gles2h.glOrthof( -0.375, size.x - 0.375, size.y - 0.375, -0.375, nearPlane, farPlane );
	}
	...
	{
		gles2h.glOrthof( param[0], param[1], param[2], param[3], param[4], param[5]);
	}
	...
	inline void glFrustum( const TooN::Vector<4, P, A> & params, const float width, const float height, const float nearPlane = 0.1, const float farPlane = 100)
	{
		...
		gles2h.glFrustumf( left, right, bottom, top, nearPlane, farPlane );
	}
	...
	{
		gles2h.glFrustumf( param[0], param[1], param[2], param[3], param[4], param[5]);
	}
	...
	{
		gles2h.glColor4f(v[0], v[1], v[2], 1);
	}
	...
	{
		gles2h.glColor4f(v[0], v[1], v[2], v[3]);
	}
	...
	{
		switch(v.size()){
		case 3: gles2h.glColor4f(v[0], v[1], v[2], 1);
			break;
		case 4: gles2h.glColor4f(v[0], v[1], v[2], v[3]);
			break;
		}
	}
	...
	{
		gles2h.glColor4f(((GLfloat)c.red)/255.0f, ((GLfloat)c.green)/255.0f, ((GLfloat)c.blue)/255.0f, 1.0f);
	}
	...
	{
		gles2h.glColor4f(c.red, c.green, c.blue, 1);
	}
	...
	{
		gles2h.glColor4f(((GLfloat)c.red)/255.0f, ((GLfloat)c.green)/255.0f, ((GLfloat)c.blue)/255.0f, ((GLfloat)c.dummy)/255.0f);
	}
	...
	{
		gles2h.glColor4f(((GLfloat)c.red)/255.0f, ((GLfloat)c.green)/255.0f, ((GLfloat)c.blue)/255.0f, ((GLfloat)c.alpha)/255.0f);
	}
	...
	{
		gles2h.glColor4f(c.red, c.green, c.blue, c.alpha);
	}
