  ...
  //if(false)
  {
    ...
    ImageRef ir = irTopLeft;
...
#ifdef __ANDROID__
    	int maxnum = max((mim.size().x-10)*(mim.size().y-10), 1);
        //int maxnum = max((mim.size().x)*(mim.size().y), 1);
    	//__android_log_print(ANDROID_LOG_INFO, "maxnum", "%d", maxnum);
    	//__android_log_print(ANDROID_LOG_INFO, "mimsize", "%d", mim.size().y);

        //GLfloat col[4*maxnum];
        //GLfloat vetx[2*maxnum];

        int count = 0;
        int nGate = GV2.GetInt("CameraCalibrator.MeanGate", 10, SILENT);
        do
        {
        	//__android_log_print(ANDROID_LOG_INFO, "check corner", "%d %d", ir.x, ir.y);
          if(IsCorner(imBlurred, ir, nGate))
    	{
        	  //__android_log_print(ANDROID_LOG_INFO, "found corner", "%d %d", ir.x, ir.y);
        ...
        /*if(count>0)
        {
    		...
    		glVertexPointer(2, GL_FLOAT, 0, vetx);
