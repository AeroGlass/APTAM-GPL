//original with no copyright (CC0) from https://github.com/jodem/glText-Android
...
public class Vertices {
   ...
   public Vertices(int textshader, int maxVertices, int maxIndices, boolean hasColor, boolean hasTexCoords, boolean hasNormals)  {
      this(textshader,  maxVertices, maxIndices, hasColor, hasTexCoords, hasNormals, false );  // Call Overloaded Constructor
   }
   public Vertices(int textshader, int maxVertices, int maxIndices, boolean hasColor, boolean hasTexCoords, boolean hasNormals, boolean use3D)  {
   ...
   public void bind(int shaderid)  {
      ...
      if ( hasColor )  {                              // IF Vertices Have Color
    	  //GLES20.glEnableClientState( GLES20.GL_COLOR_ARRAY );  // Enable Color in Vertices
         //vertices.position( positionCnt );            // Set Vertex Buffer to Color
         //GLES20.glColorPointer( COLOR_CNT, GLES20.GL_FLOAT, vertexSize, vertices );  // Set Color Pointer
      ...
      /*if ( hasNormals )  {
    	  GLES20.glEnableClientState( GLES20.GL_NORMAL_ARRAY );  // Enable Normals in Vertices
         ...
         GLES20.glNormalPointer( GLES20.GL_FLOAT, vertexSize, vertices );  // Set Normals Pointer
      }*/
   }
   ...
   public void unbind()  {
      /*if ( hasColor )                                 // IF Vertices Have Color
    	  GLES20.glDisableClientState( GLES20.GL_COLOR_ARRAY );  // Clear Color State

      if ( hasTexCoords )                             // IF Vertices Have Texture Coords
    	  GLES20.glDisableClientState( GLES20.GL_TEXTURE_COORD_ARRAY );  // Clear Texture Coords State

      if ( hasNormals )                               // IF Vertices Have Normals
    	  GLES20.glDisableClientState( GLES20.GL_NORMAL_ARRAY );  // Clear Normals State*/
   }
