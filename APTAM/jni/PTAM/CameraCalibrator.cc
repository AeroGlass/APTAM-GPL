...
CameraCalibrator::CameraCalibrator()
  :mGLWindow(mVideoSource.Size(), "Camera Calibrator"), mCamera("Camera",mVideoSource.Size(),ATANCamera::mvDefaultParams)
{
  mbDone = false;
      ...
      if(!*mgvnOptimizing)
	{
	  ...
	  if(c.MakeFromImage(imFrameBW))
	      ...
	      if(mbGrabNextFrame)
		{
		  ...
		  {
			  std::stringstream ss;
			  ss << GLWindow2::getFDir() << "im-" << imnum << ".bmp";
#ifdef __ANDROID__
			  __android_log_print(ANDROID_LOG_INFO, "Calib", "save file to: %s", ss.str().c_str());
#endif
			  img_save(imFrameBW, ss.str());
			  imnum++;
		  }
...
void CameraCalibrator::Reset()
{
  *mCamera.mgvvCameraParams = ATANCamera::mvDefaultParams;
  mCamera.mvCameraParams = ATANCamera::mvDefaultParams;
...
void CameraCalibrator::GUICommandHandler(string sCommand, string sParams)  // Called by the callback func..
{
  ...
  if(sCommand=="CameraCalibrator.SaveCalib")
      ...
      if(ofs.good())
	{
	  ...
	  cout << "  .. saved."<< endl;
#ifdef __ANDROID__
	  __android_log_print(ANDROID_LOG_INFO, "Calibrator", "Camera Parameters saved");
#endif
	}
      else
	{
...
#ifdef __ANDROID__
	  __android_log_print(ANDROID_LOG_INFO, "Calibrator", "Error saving Camera Parameters");
#endif
	}
      mbDone = true;
    }
