...
{
  DefaultPrecision dBlurSigma = 2.0;
  ...
  int nOffset;
  {
    ...
    CVD::transform(mimSharedSourceTemplate, imTwiceToBlur,
		   M2Inverse(m2Warp),
		   vec(mimSharedSourceTemplate.size() - ImageRef(1,1)) * (DefaultPrecision)0.5,
		   vec(imTwiceToBlur.size() - ImageRef(1,1)) * (DefaultPrecision)0.5);
...
{ 
  ...
  Vector<2> v2BR = v2TL + vec(mimTemplate.size() - ImageRef(1,1)) + vec(mimTemplate.size());
